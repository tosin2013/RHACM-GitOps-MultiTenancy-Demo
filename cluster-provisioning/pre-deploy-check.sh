...
# Forked from: https://github.com/tosin2013/rhacm-workshop/blob/master/01.RHACM-Installation/cluster-provisioning/pre-deploy-check.sh
#
...
echo "  blue-cluster (m6i.2xlarge) needs 8 vCPUs"
echo "  red-cluster (m6i.2xlarge) needs 8 vCPUs"
echo "  Total needed: 16 vCPUs"
...
if oc get clusterimageset img4.21.20-multi-appsub &>/dev/null; then
  echo "  [OK] ClusterImageSet img4.21.20-multi-appsub exists"
else
  echo "  WARNING: Default ClusterImageSet not found. Available:"
  oc get clusterimageset --no-headers | grep multi-appsub | sort -V | tail -5
fi
...
if (( $(echo "$FREE_EIPS >= $REQUIRED_EIPS" | bc -l) )); then
  ...
  echo "  Run this script again after cleanup or wait for quota approval."
fi
